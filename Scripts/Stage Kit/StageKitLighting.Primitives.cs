...
namespace YALCY
{
        ...
        public override void Enable()
        {
            ...
            Task.Run(async () =>
            {
                while (!_cancellationTokenSource.Token.IsCancellationRequested)
                {
                    ...
                    var secondsPerBeat = 60f/ UdpIntake.Buffer[(int)UdpIntake.ByteIndexName.BeatsPerMinute];
    ...
    public class ListenPattern : StageKitLighting
    {
        ...
        private void HandleBeatlineEvent(UdpIntake.BeatByte eventName)
        {
            ...
            if (((_listenType & ListenTypes.MajorBeat) == 0 || eventName != UdpIntake.BeatByte.Measure) &&
                ((_listenType & ListenTypes.MinorBeat) == 0 || eventName != UdpIntake.BeatByte.Strong))
            {
                return;
            }
        ...
        private void HandleDrumEvent(UdpIntake.DrumNotesByte eventName)
        {
            if (!_enabled)
            {
                return;
            ...
            if ((_listenType & ListenTypes.RedFretDrums) == 0 || (eventName &  UdpIntake.DrumNotesByte.RedDrum) == 0)
            {
                return;
            }
        ...
        public void HandleKeyFrameEvent(UdpIntake.KeyFrameByte eventName)
        {
            if (!_enabled)
            {
                return;
            ...
            if ((_listenType & ListenTypes.Next) == 0 || eventName != UdpIntake.KeyFrameByte.KeyframeNext)
            {
                return;
            }
